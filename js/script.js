...
share_buttons_already_here = false;
aelios = {
    o : {
        mapLoaded : 0
        ,firstTime : true
...
//        });
	    ...
	    $('html').bind('click',function(e){
			if($(this).hasClass('avgrund-active') && !$(e.target).parents('.avgrund-popup').length && !$(e.target).is('.avgrund-popup')){
				closeDialog();
			}
	    })
        $('#search').bind('click',aelios.search);
        $('#info').bind('click',openDialog);
...
function getShareButtons(){
            ...
            (function(d, s, id) {
              ...
              js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=154268327997027";
...
function openDialog() {
			...
			getShareButtons();
			share_buttons_already_here = true;
}
